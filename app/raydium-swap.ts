...
import { SINGER_PRIVATE_KEY } from "./constants"
...
const payer = Keypair.fromSecretKey(bs58.decode(SINGER_PRIVATE_KEY))
...
async function swapSolToUsdc() {
	...
	try {
		tAta = await getOrCreateAssociatedTokenAccount(
			connection,
			payer,
			new PublicKey("5QiDKfdEHAqAMEDMEs48izW8WFKdGnMA3BmoHrQgyhcw"),
			payer.publicKey
		)
		sAta = await getOrCreateAssociatedTokenAccount(
			connection,
			payer,
